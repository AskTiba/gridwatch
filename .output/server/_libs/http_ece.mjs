...
function requireEce() {
  ...
  function HMAC_hash(key, input) {
    var hmac = crypto$1.createHmac("sha256", key);
    hmac.update(input);
    return hmac.digest();
  }
  ...
  function decryptRecord(key, counter, buffer, header, last) {
    ...
    var gcm = crypto$1.createDecipheriv(AES_GCM, key.key, nonce);
  ...
  function encryptRecord(key, counter, buffer, pad, header, last) {
    ...
    pad = pad || 0;
    var nonce = generateNonce(key.nonce, counter);
    var gcm = crypto$1.createCipheriv(AES_GCM, key.key, nonce);
    var ciphertext = [];
  ...
  function encrypt(buffer, params, keyLookupCallback) {
    ...
    var header = parseParams(params);
    if (!header.salt) {
      header.salt = crypto$1.randomBytes(KEY_LENGTH);
    }
    var result;
    if (header.version === "aes128gcm") {
